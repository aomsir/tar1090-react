// Verbatim port of tar1090 registrations_*.js allocation algorithms.
// Reverse-engineered ICAO-address -> registration mappings; do not alter constants.
//
...
export const registrationFromHexId: (hexid: string) => string | null = (() => {
  ...
  ].map((m) => m as unknown as StrideMapping);
  ...
  ].map((m) => m as unknown as NumericMapping);
  ...
  function lookup(hexid_str: string): string | null {
    const hexid = +('0x' + hexid_str);
  ...
  function numeric_reg(hexid: number): string | undefined {
