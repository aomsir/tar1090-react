...
// Ported verbatim from the original tar1090 source
// (flags_f7cd6d83ee8fe9acd967ecd67d365e19.js). Mostly generated from the
// assignment table in the appendix to Chapter 9 of Annex 10 Vol III, Second
// Edition, July 2007 (with amendments through 88-A, 14/11/2013).
const ICAO_Ranges: IcaoRange[] = [
  ...
  { start: 0x899000, end: 0x8993ff, country: 'China', country_code: 'cn' },
...
// Ported verbatim from the original source. The original object has only
// `country` and `country_code`; `start`/`end` are added here as 0 placeholders
// to satisfy the IcaoRange type (the original relied on these being absent).
...
// Given a (hex string) ICAO address, return an object describing that ICAO
// range. Always returns a non-null object.
export function findCountry(icao: string): IcaoRange {
  const hexa = Number('0x' + icao);
  for (const r of ICAO_Ranges) {
    if (hexa >= r.start && hexa <= r.end) return r;
  }
