...
function topN(map: Map<string, number>, n: number): { name: string; count: number }[] {
  const entries = Array.from(map.entries())
    .filter(([name]) => name !== '')
    .sort((a, b) => b[1] - a[1]);
  const top = entries.slice(0, n).map(([name, count]) => ({ name, count }));
  const rest = entries.slice(n).reduce((s, [, c]) => s + c, 0);
...
export function computeHistoryStats(
  ...
  return {
    ...
    militaryCount,
    peakOnline,
    typeDistribution: topN(typeMap, 10),
    airlineDistribution: topN(airlineMap, 10),
    countryDistribution: topN(countryMap, 8),
